...
class Airfield: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate {
...
//    private var htmlContent: NSString!
    ...
    func setupPicker () {
        ...
        airfieldPicker.backgroundColor = mainColor
    ...
    func pickUpValue(textField: UITextField) {
        ...
        let doneButton = UIBarButtonItem(title: "Gotowe", style: UIBarButtonItemStyle.plain, target: self, action: #selector(doneTapped))
    ...
    func loadMETAR () {
        self.metarOutput.text = "Pobieram depesze METAR"
    ...
    func loadTAF() {
        self.tafOutput.text = "Pobieram depesze TAF"
        ...
        let task = urlSession.dataTask(with: request) { (data, response, error) in
            if let error = error {
                ...
                if let start = text.range(of: currentName),
                    ...
                    self.fetchedTAF = TAFItem(title: self.pickedAirfield, description: ": Brak aktualnej depeszy")
                }
            }
